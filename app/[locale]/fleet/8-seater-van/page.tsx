...
// HERO SECTION
// ============================================================================
function HeroSection({ t }: { t: ReturnType<typeof useTranslations<'fleet.eightSeaterVan'>> }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute bottom-1/4 -right-20 w-80 h-80 bg-violet-500/10 rounded-full blur-3xl" />
      </div>
...
// VEHICLES SECTION
// ============================================================================
function VehiclesSection({ t }: { t: ReturnType<typeof useTranslations<'fleet.eightSeaterVan'>> }) {
  const ref = useRef(null);
      ...
      <div className={`${isReversed ? 'lg:order-2' : ''}`}>
        <div className={`relative aspect-[4/3] bg-gradient-to-br ${vehicle.bgColor} rounded-3xl flex items-center justify-center border-2 ${vehicle.borderColor} transition-colors overflow-hidden`}>
          ...
          <div className="absolute top-4 left-4">
            <span className={`px-3 py-1 text-xs font-bold text-white bg-gradient-to-r ${vehicle.color} rounded-full flex items-center gap-1`}>
              <UsersIcon className="w-3 h-3" />
              {t('labels.eightPassengers')}
            </span>
          </div>
          <div className="text-center p-8">
            <VanIcon className="w-28 h-28 text-gray-400 mx-auto mb-4" />
...
// CTA SECTION
// ============================================================================
function CTASection({ t }: { t: ReturnType<typeof useTranslations<'fleet.eightSeaterVan'>> }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.3 });
    ...
    <section ref={ref} className="py-20 lg:py-28 bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
          transition={{ duration: 0.6 }}
          className="text-center bg-gradient-to-br from-indigo-600 via-indigo-700 to-violet-600 rounded-3xl p-10 lg:p-16 shadow-2xl"
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center px-8 py-4 text-base font-semibold text-indigo-700 bg-white rounded-xl hover:bg-gray-100 transition-colors shadow-lg w-full sm:w-auto"
            >
              <WhatsAppIcon className="w-5 h-5 mr-2" />
              {t('cta.whatsapp')}
            </a>
